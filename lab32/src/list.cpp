...
void List::change_vals() {
    unique_ptr<int> first(new int(15));
    pAgency[0]->set_age(*first);

    auto_ptr<int> second (new int(20));
    pAgency[1]->set_age(*second);

    shared_ptr<int> third (new int(125));
    pAgency[2]->set_age(*third);

    std::weak_ptr<int> weak;
    auto to_test = std::make_shared<int>(42);
    weak = to_test;

    if (auto spt = weak.lock()) {
        pAgency[3]->set_age(*spt);
    }
    else {
        std::cout << "Weak isn't active\n";
    }
}
